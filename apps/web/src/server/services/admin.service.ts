...
import { getRound, invalidateRoundCache } from "./round.service";
import { resetSession } from "./sessions.service";

export async function importRoster(body: RosterImport): Promise<RosterImportResponse> {
  ...
  const byCode = new Map(
    (await prisma.team.findMany({ where: { code: { in: codes } } })).map(
      (t) => [t.code, t],
    ),
  );

  const clashes = await prisma.team.findMany({
...
function toStatusResponse(round: {
  number: number;
  status: string;
  startedAt: Date | null;
  pausedAt: Date | null;
...
export async function getAdminRoundStatus(): Promise<AdminRoundStatusResponse> {
  const round = await getRound();
  return toStatusResponse(round);
}

export async function updateAdminConfig(input: AdminRoundConfig): Promise<AdminRoundStatusResponse> {
  const round = await getRound();
  if (round.status === "ACTIVE") {
    throw new Error("Round config cannot change while the round is active");
  }
  ...
  await invalidateRoundCache();
  return toStatusResponse(updated);
}

export async function startAdminRound(): Promise<AdminRoundStatusResponse> {
  const round = await getRound();
  if (round.status !== "IDLE" && round.status !== "ENDED") {
    throw new Error(`The round cannot start from status "${round.status}"`);
  }

  const env = getEnv();
  const durationSeconds = env.ROUND_DURATION_SECONDS ?? 1800;
...
export async function pauseAdminRound(): Promise<AdminRoundStatusResponse> {
  const round = await getRound();
  if (round.status !== "ACTIVE") {
    throw new Error(`Only an active round can be paused (status: "${round.status}")`);
  }
...
export async function resumeAdminRound(): Promise<AdminRoundStatusResponse> {
  const round = await getRound();
  if (round.status !== "PAUSED" || !round.pausedAt) {
    throw new Error(`Only a paused round can be resumed (status: "${round.status}")`);
  }
...
export async function endAdminRound(): Promise<AdminRoundStatusResponse> {
  const round = await getRound();
  if (round.status === "IDLE" || round.status === "ENDED") {
    throw new Error(`The round cannot end from status "${round.status}"`);
  }
...
export async function listAdminTeams(): Promise<AdminTeamsResponse> {
  ...
  const rows: AdminTeamRow[] = teams.map((t) => ({
    ...
    room: t.room,
    sessions: t.sessions.map((s) => toSessionRow(s)),
...
export async function resetAdminTeam(input: AdminTeamReset): Promise<{ reset: number }> {
  ...
  if (sessions.length === 0) {
    throw new Error("No sessions for this team");
  }
